...
// during signup we are giving id, pass, name and userRole.
  ...
  User.findOne({
    ...
    if (user) {
      ...
      bcrypt.hash(req.body.password, 12).then((hashedPassword) => {
        Role.findAll({ // finding all roles that the user specified in req, so that we can set it later
          where: {
            name: {
          ...
          Cart.create() // it will create cart for this particular user and store its id in it.
            .then((cart) => {
              User.create({
                name: req.body.name,
